		...
		didSet {
			...
			UIView.animate(
				...
				completion: { completed in
					...
					self.moveSheet(to: self.config.nextY(self, from: self.sheetViewController.view.frame.minY, panDirection: .up))
				}
			)
		}
	}
...
public extension BottomSheetController {
	func expand() {
		moveSheet(to: config.minYBound(self))
	}
	func collapse() {
		moveSheet(to: config.maxYBound(self))
	}
}
...
private extension BottomSheetController {
	func prepareSheetForPresentation() {
		...
		self.isTotallyExpanded = initialY == config.minYBound(self)
		self.isTotallyCollapsed = initialY == config.maxYBound(self)
		self.handleBackgroundView()
	}
	...
	func moveSheet(to y: CGFloat, velocity: CGPoint = .zero) {
		...
		self.isTotallyExpanded = y == config.minYBound(self)
		self.isTotallyCollapsed = y == config.maxYBound(self)
	...
	func handleBackgroundView() {
		guard config.disableBackground(self) else { return }
		let screenHeight = UIScreen.main.bounds.height
		let height = screenHeight - ceil(sheetViewController.view.frame.origin.y)
		let paddingBottom = screenHeight - self.config.maxYBound(self)
		...
		topPadding += self.config.minYBound(self)
		let maxAlphaBackground = self.config.maxAlphaBackground(self)
...
private extension BottomSheetController {
	@objc func panGestureHandler(_ recognizer: UIPanGestureRecognizer) {
		...
		let minYBound = config.minYBound(self)
		let maxYBound = config.maxYBound(self)
		...
		case .ended:
			let targetY = config.nextY(self, from: sheetViewController.view.frame.minY, panDirection: direction)
